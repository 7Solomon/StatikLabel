...
def detect_scheiben(connections, initial_scheiben=None):
    ...
    # Always find triangles
    triangles = find_triangles()
    print(triangles)
    # Start with initial_scheiben if provided, and add found triangles
    if initial_scheiben:
    ...
    # If no Scheiben found, return all nodes as separate sets
    if not final_scheiben:
        return {'final_scheiben': [{node} for node in graph], 'graph': graph}

    # Expand all Scheiben
    final_scheiben = [expand_scheibe(scheibe) for scheibe in final_scheiben]
    ...
    final_scheiben.extend(isolated_scheiben)

    return {'final_scheiben': final_scheiben, 'graph': graph}
